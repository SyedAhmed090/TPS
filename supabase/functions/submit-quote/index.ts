...
const ALLOWED_TYPES = ['application/pdf','image/svg+xml','image/png','image/jpeg','application/postscript','image/vnd.adobe.photoshop','application/octet-stream']
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!quantity || quantity < 1) errors.quantity = 'Valid quantity is required'
