...
const STATUS_COLORS = { pending: '#f59e0b', shipped: '#06b6d4', delivered: '#22c55e' }
...
export default function SampleRequests() {
  ...
  async function markShipped(id) {
    await supabase.from('sample_requests').update({ status: 'shipped', shipped_at: new Date().toISOString() }).eq('id', id)
    setRows(prev => prev.map(r => r.id === id ? { ...r, status: 'shipped', shipped_at: new Date().toISOString() } : r))
  }

  async function markDelivered(id) {
    await supabase.from('sample_requests').update({ status: 'delivered' }).eq('id', id)
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--white)', border: '1px solid rgba(11,26,46,0.1)', overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Date','Name','Email','Location','Patch Types','Status','Actions'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={r.id} style={{ background: i % 2 === 1 ? 'var(--cream)' : 'var(--white)' }}>
                  ...
                  <td style={td}><strong>{r.name}</strong>{r.company && <div style={{ fontSize: '0.75rem', color: 'var(--gray-mid)' }}>{r.company}</div>}</td>
                  <td style={td}>{r.email}</td>
                  <td style={td}>{r.city}, {r.state}</td>
                  <td style={{ ...td, fontSize: '0.8rem' }}>{(r.patch_types || []).join(', ') || '—'}</td>
                  <td style={td}>
                  ...
                  <td style={td}>
                    {r.status === 'pending' && (
                      <button onClick={() => markShipped(r.id)} style={{ padding: '4px 10px', background: 'transparent', color: 'var(--navy)', border: '1px solid rgba(11,26,46,0.3)', fontSize: '0.72rem', fontFamily: 'var(--font-heading)', letterSpacing: '0.05em', cursor: 'pointer', marginRight: 4 }}>
                        Mark Shipped
                      </button>
                    )}
                    {r.status === 'shipped' && (
                      <button onClick={() => markDelivered(r.id)} style={{ padding: '4px 10px', background: 'transparent', color: 'var(--navy)', border: '1px solid rgba(11,26,46,0.3)', fontSize: '0.72rem', fontFamily: 'var(--font-heading)', letterSpacing: '0.05em', cursor: 'pointer' }}>
                        Mark Delivered
                      </button>
                    )}
                  </td>
